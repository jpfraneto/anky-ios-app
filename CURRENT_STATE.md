...
| GET | `/swift/v2/writings` | Fetch persisted writing history, including processed Anky artifacts like reflection, title, id, and image URL for completed Ankys |
...
5. **Live-verify the real-anky submit loop**: Run a real `POST /api/anky/submit` session end-to-end against the backend, confirm the reflection/image stream arrives on device, and confirm `/swift/v2/writings` later returns the same anky without relying on the local merge fallback.
